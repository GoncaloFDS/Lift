...
layout(location = 1) rayPayloadInEXT LightNode light_node_;
...
void main() {
    ...
    uint seed = tea(gl_LaunchIDEXT.y * gl_LaunchSizeEXT.x + gl_LaunchIDEXT.x, ubo_.frame * ubo_.number_of_samples + light_node_.index);

    HitSample hit = scatter(material, gl_WorldRayDirectionEXT, normal, tex_coords, seed);

    light_nodes_[light_node_.index].position = vec4(gl_WorldRayOriginEXT  + gl_WorldRayDirectionEXT * gl_HitTEXT, 0);
