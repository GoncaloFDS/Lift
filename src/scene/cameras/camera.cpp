...
void lift::Camera::onUpdate() {
    ...
    if (changed_) {
        ...
        const auto k_w_length = length(vector_w_);
        ...
        const auto v_length = k_w_length * tanf(0.5f * fovy_ * pi<float>() / 180.f);
        vector_v_ *= v_length;
...
void lift::Camera::strafe(const float dx, const float dy) {
    const auto mat = translate(mat4(1.0f), dx * mouse_strafe_speed_ * norm_vector_u_) *
        translate(mat4(1.0f), dy * mouse_strafe_speed_ * norm_vector_v_);
