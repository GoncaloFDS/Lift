...
Options::Options(const int argc, const char *argv[]) {
  ...
  benchmark_options.add_options()("next-scenes", bool_switch(&benchmarkNextScenes)->default_value(false),
                                  "Load the next scene once the sample or time limit is reached.")(
    "max-time", value<uint32_t>(&benchmarkMaxTime)->default_value(60),
    "The benchmark time limit per scene (in seconds).");

  options_description renderer("Renderer options");
  renderer.add_options()("samples", value<uint32_t>(&samples)->default_value(4),
                         "Set the number of ray samples per pixel.")(
    "bounces", value<uint32_t>(&bounces)->default_value(16),
    "Set the maximum number of bounces per ray.")("max-samples", value<uint32_t>(&maxSamples)->default_value(64 * 1024),
  ...
  window.add_options()("width", value<uint32_t>(&width)->default_value(1280), "Set framebuffer width.")(
  ...
  desc.add_options()("help", "Display help message.")("benchmark", bool_switch(&benchmark)->default_value(false),
