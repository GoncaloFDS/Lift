...
auto lift::pixelFormatSize(lift::BufferImageFormat format) -> size_t {
	switch (format) {
		case BufferImageFormat::UNSIGNED_BYTE_4: return sizeof(char) * 4;
		case BufferImageFormat::FLOAT_3: return sizeof(float) * 3;
		case BufferImageFormat::FLOAT_4: return sizeof(float) * 4;
	}
	LF_ASSERT(false, "Invalid BufferImageFormat");
}

lift::OpenGLContext::OpenGLContext(std::shared_ptr<Window> window_handle,
								   BufferImageFormat format)
	: window_handle_(static_cast<GLFWwindow*>(window_handle->getNativeWindow())), image_format_(format) {
...
auto lift::createGLProgram(const std::string& vert_source, const std::string& frag_source) -> GLuint {
...
auto lift::getGLUniformLocation(GLuint program, const std::string& name) -> GLint {
...
const std::string lift::OpenGLContext::s_vert_source = R"(
#version 330 core
...
const std::string lift::OpenGLContext::s_frag_source = R"(
#version 330 core
...
void lift::OpenGLContext::display(ivec2 screen_res, ivec2 frame_buffer_res, uint32_t pbo) {
