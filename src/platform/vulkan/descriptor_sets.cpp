...
DescriptorSets::DescriptorSets(const DescriptorPool &descriptor_pool, const DescriptorSetLayout &layout,
                               std::map<uint32_t, VkDescriptorType> binding_types, const size_t size) :
...
VkWriteDescriptorSet DescriptorSets::bind(const uint32_t index, const uint32_t binding,
                                          const VkDescriptorBufferInfo &buffer_info, const uint32_t count) const {
...
VkWriteDescriptorSet DescriptorSets::bind(const uint32_t index, const uint32_t binding,
                                          const VkDescriptorImageInfo &image_info, const uint32_t count) const {
  VkWriteDescriptorSet descriptor_write = {};
  descriptor_write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
  descriptor_write.dstSet = descriptor_sets_[index];
  descriptor_write.dstBinding = binding;
...
VkWriteDescriptorSet DescriptorSets::bind(uint32_t index, uint32_t binding,
...
void DescriptorSets::updateDescriptors(const std::vector<VkWriteDescriptorSet> &descriptor_writes) {
  vkUpdateDescriptorSets(descriptor_pool_.device().handle(), static_cast<uint32_t>(descriptor_writes.size()),
