...
class BottomLevelAccelerationStructure final : public AccelerationStructure {
  ...
  BottomLevelAccelerationStructure(const class DeviceProcedures &device_procedures,
                                   const std::vector<VkGeometryNV> &geometries, bool allow_update);
  ...
  static VkGeometryNV createGeometryAabb(const assets::Scene &scene, uint32_t aabb_offset, uint32_t aabb_count,
                                         bool is_opaque);
