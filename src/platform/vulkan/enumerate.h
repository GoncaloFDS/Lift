...
inline std::vector<TValue> getEnumerateVector(VkResult(enumerate)(uint32_t*, TValue*), std::vector<TValue>& vector) {
    uint32_t count = 0;
    vulkanCheck(enumerate(&count, nullptr),
                "enumerate");

    vector.resize(count);
    vulkanCheck(enumerate(&count, vector.data()),
                "enumerate");
...
inline std::vector<TValue> getEnumerateVector(THandle handle,
...
inline std::vector<TValue> getEnumerateVector(THandle handle,
                                              ...
                                              std::vector<TValue>& vector) {
    uint32_t count = 0;
    vulkanCheck(enumerate(handle, &count, nullptr),
                "enumerate");

    vector.resize(count);
    vulkanCheck(enumerate(handle, &count, vector.data()),
                "enumerate");
