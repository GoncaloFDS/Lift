...
void BottomLevelAccelerationStructure::generate(VkCommandBuffer command_buffer, Buffer &scratch_buffer,
                                                VkDeviceSize scratch_offset, DeviceMemory &result_memory,
                                                VkDeviceSize result_offset, bool update_only) const {
  ...
  device_procedures_.vkCmdBuildAccelerationStructureNV(command_buffer, &buildInfo, nullptr, 0, update_only, handle(),
                                                       previous_structure, scratch_buffer.handle(), scratch_offset);
}

VkGeometryNV BottomLevelAccelerationStructure::createGeometry(const assets::Scene &scene, uint32_t vertex_offset,
                                                              uint32_t vertex_count, uint32_t index_offset,
                                                              uint32_t index_count, bool is_opaque) {
...
VkGeometryNV BottomLevelAccelerationStructure::createGeometryAabb(const assets::Scene &scene, uint32_t aabb_offset,
