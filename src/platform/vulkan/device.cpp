...
findQueue(const std::vector<VkQueueFamilyProperties> &queue_families, const std::string &name,
          const VkQueueFlags required_bits, const VkQueueFlags excluded_bits) {
  const auto family = std::find_if(queue_families.begin(), queue_families.end(),
...
Device::Device(VkPhysicalDevice physical_device, const class Surface &surface) :
  ...
  const std::set<uint32_t> unique_queue_families = {graphics_family_index_, compute_family_index_,
                                                    present_family_index_, transfer_family_index_};
