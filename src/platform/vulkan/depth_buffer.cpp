...
VkFormat findSupportedFormat(const Device &device, const std::vector<VkFormat> &candidates, const VkImageTiling tiling,
...
DepthBuffer::DepthBuffer(CommandPool &command_pool, const VkExtent2D extent) :
  ...
  image_ = std::make_unique<Image>(device, extent, format_, VK_IMAGE_TILING_OPTIMAL,
