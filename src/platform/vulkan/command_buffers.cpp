...
CommandBuffers::~CommandBuffers() {
  if (!command_buffers_.empty()) {
    vkFreeCommandBuffers(command_pool_.device().handle(), command_pool_.handle(),
...
VkCommandBuffer CommandBuffers::begin(const size_t i) {
  ...
  vulkanCheck(vkBeginCommandBuffer(command_buffers_[i], &begin_info), "begin recording command buffer");

  return command_buffers_[i];
}

void CommandBuffers::end(const size_t i) {
  vulkanCheck(vkEndCommandBuffer(command_buffers_[i]), "record command buffer");
}
