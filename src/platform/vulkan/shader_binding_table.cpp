...
size_t copyShaderData(uint8_t *const dst, const RayTracingProperties &ray_tracing_properties,
                      const std::vector<ShaderBindingTable::Entry> &entries, const size_t entry_size,
...
ShaderBindingTable::ShaderBindingTable(const DeviceProcedures &device_procedures,
                                       ...
                                       const std::vector<Entry> &miss_programs, const std::vector<Entry> &hit_groups) :
  ...
  vulkanCheck(device_procedures.vkGetRayTracingShaderGroupHandlesNV(
                device.handle(), ray_tracing_pipeline.handle(), 0, static_cast<uint32_t>(groupCount),
