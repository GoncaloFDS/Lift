...
Texture::Texture(const class Device& device, const VkExtent2D extent, const VkFormat format) :
	Texture(device,
			extent,
			format,
...
Texture::Texture(
	...
	Vulkan_Check(vkCreateImage(device.Handle(), &imageInfo, nullptr, &image_),
				 "create image");
}
...
DeviceMemory Texture::AllocateMemory(const VkMemoryPropertyFlags properties) const {
	...
	Vulkan_Check(vkBindImageMemory(device_.Handle(), image_, memory.Handle(), 0),
				 "bind image memory");
...
void Texture::TransitionImageLayout(CommandPool& commandPool, VkImageLayout newLayout) {
...
void Texture::CopyFrom(CommandPool& commandPool, const Buffer& buffer) {
	SingleTimeCommands::Submit(commandPool, [&](VkCommandBuffer commandBuffer) {
