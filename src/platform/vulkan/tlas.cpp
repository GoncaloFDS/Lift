...
VkAccelerationStructureCreateInfoNV getCreateInfo(const size_t instance_count, const bool allow_update) {
  const auto flags = allow_update ? VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_NV :
...
void TopLevelAccelerationStructure::generate(VkCommandBuffer command_buffer, Buffer &scratch_buffer,
                                             VkDeviceSize scratch_offset, DeviceMemory &result_memory,
                                             VkDeviceSize result_offset, Buffer &instance_buffer,
                                             DeviceMemory &instance_memory, VkDeviceSize instance_offset,
  ...
  device_procedures_.vkCmdBuildAccelerationStructureNV(command_buffer, &buildInfo, instance_buffer.handle(),
                                                       instance_offset, update_only, handle(), previousStructure,
                                                       scratch_buffer.handle(), scratch_offset);
}
...
TopLevelAccelerationStructure::createGeometryInstance(const BottomLevelAccelerationStructure &bottom_level_as,
  ...
  vulkanCheck(deviceProcedures.vkGetAccelerationStructureHandleNV(device.handle(), bottom_level_as.handle(),
  ...
  geometryInstance.mask = 0xFF;  // The visibility mask is always set of 0xFF, but if some instances would need to be
                                 // ignored in some cases, this flag should be passed by the application.
  geometryInstance.instanceOffset = hit_group_index;  // Set the hit group index, that will be used to find the shader
                                                      // code to execute when hitting the geometry.
  geometryInstance.flags = VK_GEOMETRY_INSTANCE_TRIANGLE_CULL_DISABLE_BIT_NV;  // Disable culling - more fine control
