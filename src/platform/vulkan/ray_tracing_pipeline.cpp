...
RayTracingPipeline::RayTracingPipeline(const DeviceProcedures &device_procedures, const SwapChain &swap_chain,
                                       const TopLevelAccelerationStructure &acceleration_structure,
                                       const ImageView &accumulation_image_view, const ImageView &output_image_view,
  ...
  vulkanCheck(
    device_procedures.vkCreateRayTracingPipelinesNV(device.handle(), nullptr, 1, &pipeline_info, nullptr, &pipeline_),
