...
void Instance::getVulkanDevices() {
    ...
    LF_ASSERT(!physical_devices_.empty(), "found no vulkan physical devices");
}
...
void Instance::checkVulkanMinimumVersion(const uint32_t min_version) {
    uint32_t version;
    vulkanCheck(vkEnumerateInstanceVersion(&version), "query instance version");
...
void Instance::checkVulkanValidationLayerSupport(const std::vector<const char *> &validation_layers) {
    ...
    for (const char *layer : validation_layers) {
        ...
        LF_ASSERT(result != available_layers.end(),
                  "could not find the requested validation layer: '{0}",
                  std::string(layer));
    }
}
