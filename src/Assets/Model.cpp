...
struct hash<assets::Vertex> final {
  size_t operator()(assets::Vertex const &vertex) const noexcept {
    return combine(
      ...
      combine(hash<vec3>()(vertex.normal), combine(hash<vec2>()(vertex.texCoord), hash<int>()(vertex.materialIndex))));
  }
...
Model Model::loadModel(const std::string &filename) {
  ...
  if (!tinyobj::LoadObj(&obj_attrib, &obj_shapes, &obj_materials, &warn, &err, filename.c_str(),
                        material_path.c_str())) {
  ...
  for (const auto &shape : obj_shapes) {
    ...
    for (const auto &index : mesh.indices) {
      ...
      vertex.normal = {obj_attrib.normals[3 * index.normal_index + 0], obj_attrib.normals[3 * index.normal_index + 1],
                       obj_attrib.normals[3 * index.normal_index + 2]};

      if (!obj_attrib.texcoords.empty()) {
        vertex.texCoord = {obj_attrib.texcoords[2 * index.texcoord_index + 0],
  ...
  LF_INFO("Loaded model {0} in {1} -> {2} vertices and {3} materials", filename, elapsed, obj_attrib.vertices.size(),
          materials.size());
...
Model Model::createSphere(const vec3 &center, float radius, const Material &material, const bool is_procedural) {
  ...
  return Model(std::move(vertices), std::move(indices), std::vector<Material> {material},
