...
void addTriangle(std::vector<uint32_t> &indices, const uint32_t offset, const uint32_t i0, const uint32_t i1,
                 const uint32_t i2) {
...
void CornellBox::create(const float scale, std::vector<Vertex> &vertices, std::vector<uint32_t> &indices,
