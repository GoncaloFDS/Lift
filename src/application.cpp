...
assets::UniformBufferObject Application::getUniformBufferObject(VkExtent2D extent) const {
  ...
  ubo.projection =
    perspective(radians(user_settings_.fieldOfView), extent.width / static_cast<float>(extent.height), 0.1f, 10000.0f);
...
void Application::createSwapChain() {
  ...
  user_interface_ = std::make_unique<ImguiLayer>(renderer_->commandPool(), renderer_->swapChain(),
                                                 renderer_->depthBuffer(), user_settings_);
...
bool Application::onKeyPress(KeyPressedEvent &e) {
  ...
  switch (e.keyCode()) {
    ...
    case LF_KEY_F2: user_settings_.showOverlay = !user_settings_.showOverlay; break;
    case LF_KEY_F3: user_settings_.isRayTraced = !user_settings_.isRayTraced; break;
    default: break;
  }
  return false;
}
