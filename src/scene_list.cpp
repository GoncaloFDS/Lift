      ...
      if (length(center - vec3(4, 0.2f, 0)) > 0.9) {
        if (choose_mat < 0.8f)  // Diffuse
        {
          models.push_back(Model::createSphere(
            center, 0.2f, Material::lambertian(vec3(random() * random(), random() * random(), random() * random())),
            is_procedural));
...
SceneAssets SceneList::lucyInOneWeekend(CameraInitialState &camera) {
  ...
  lucy_0.transform(rotate(scale(translate(i, vec3(0, -0.08f, 0)), vec3(scale_factor)), radians(90.0f), vec3(0, 1, 0)));

  lucy_1.transform(rotate(scale(translate(i, vec3(-4, -0.08f, 0)), vec3(scale_factor)), radians(90.0f), vec3(0, 1, 0)));

  lucy_2.transform(rotate(scale(translate(i, vec3(4, -0.08f, 0)), vec3(scale_factor)), radians(90.0f), vec3(0, 1, 0)));
