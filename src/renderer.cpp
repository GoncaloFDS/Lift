...
void Renderer::beginCommand(assets::Scene &scene, size_t current_frame) {
  ...
  auto result = vkAcquireNextImageKHR(device_->handle(), swap_chain_->handle(), no_timout,
                                      current_image_available_semaphore_, nullptr, &current_image_index_);
...
void Renderer::traceCommand(VkCommandBuffer command_buffer, uint32_t image_index, assets::Scene &scene) {
  ...
  ImageMemoryBarrier::insert(command_buffer, output_image_->handle(), subresource_range, 0, VK_ACCESS_SHADER_WRITE_BIT,
  ...
  vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_RAY_TRACING_NV,
                          ray_tracing_pipeline_->pipelineLayout().handle(), 0, 1, descriptor_sets, 0, nullptr);

  device_procedures_->vkCmdTraceRaysNV(
    command_buffer, shader_binding_table_->buffer().handle(), shader_binding_table_->rayGenOffset(),
    shader_binding_table_->buffer().handle(), shader_binding_table_->missOffset(),
    shader_binding_table_->missEntrySize(), shader_binding_table_->buffer().handle(),
    shader_binding_table_->hitGroupOffset(), shader_binding_table_->hitGroupEntrySize(), nullptr, 0, 0, extent.width,
    extent.height, 1);
...
void Renderer::display(VkCommandBuffer command_buffer, uint32_t image_index) {
  ...
  ImageMemoryBarrier::insert(command_buffer, output_image_->handle(), subresource_range, VK_ACCESS_SHADER_WRITE_BIT,
                             VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                             VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);

  ImageMemoryBarrier::insert(command_buffer, swapChain().images()[image_index], subresource_range, 0,
  ...
  vkCmdCopyImage(command_buffer, output_image_->handle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                 swapChain().images()[image_index], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy_region);
}
...
void Renderer::rasterizeCommand(VkCommandBuffer command_buffer, uint32_t image_index, assets::Scene &scene) {
  ...
  vkCmdBeginRenderPass(command_buffer, &render_pass_info, VK_SUBPASS_CONTENTS_INLINE);
  {
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            graphics_pipeline_->pipelineLayout().handle(), 0, 1, descriptor_sets, 0, nullptr);
...
void Renderer::createBottomLevelStructures(VkCommandBuffer command_buffer, assets::Scene &scene) {
  ...
  for (const auto &model : scene.models()) {
    ...
    const std::vector<VkGeometryNV> geometries = {
      model.procedural() ? BottomLevelAccelerationStructure::createGeometryAabb(scene, aabb_offset, 1, true) :
                           BottomLevelAccelerationStructure::createGeometry(scene, vertex_offset, vertex_count,
                                                                            index_offset, index_count, true)};
  ...
  for (size_t i = 0; i != bottom_as_.size(); ++i) {
    bottom_as_[i].generate(command_buffer, *bottom_scratch_buffer_, scratch_offset, *bottom_buffer_memory_,
                           result_offset, false);
...
void Renderer::createTopLevelStructures(VkCommandBuffer command_buffer, assets::Scene &scene) {
  ...
  top_buffer_memory_ = std::make_unique<DeviceMemory>(top_buffer_->allocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT));
  ...
  // Generate the structures.
  top_as_[0].generate(command_buffer, *top_scratch_buffer_, 0, *top_buffer_memory_, 0, *instances_buffer_,
                      *instances_buffer_memory_, 0, false);
}
...
void Renderer::createOutputImage() {
  ...
  /////////////
  accumulation_image_ = std::make_unique<Image>(device(), extent, VK_FORMAT_R32G32B32A32_SFLOAT,
  ...
  accumulation_image_view_ = std::make_unique<ImageView>(device(), accumulation_image_->handle(),
                                                         VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT);

  /////////////
  output_image_ = std::make_unique<Image>(device(), extent, format, tiling,
  ...
  /////////////
  denoised_image_ = std::make_unique<Image>(device(), extent, VK_FORMAT_R32G32B32A32_SFLOAT, tiling,
  ...
  //        std::make_unique<ImageView>(device(), denoised_image_->handle(),
  //        format, VK_IMAGE_ASPECT_COLOR_BIT);
}

void Renderer::createRayTracingPipeline(assets::Scene &scene) {
  ray_tracing_pipeline_ =
    std::make_unique<RayTracingPipeline>(*device_procedures_, swapChain(), top_as_[0], *accumulation_image_view_,
                                         *output_image_view_, uniformBuffers(), scene);
  ...
  shader_binding_table_ = std::make_unique<ShaderBindingTable>(
    *device_procedures_, *ray_tracing_pipeline_, *properties_, ray_gen_programs, miss_programs, hit_groups);
}
