	...
	userSettings_(userSettings)
{
	...
	descriptorPool_ = std::make_unique<Vulkan::DescriptorPool>(device, descriptorBindings, 1);
	renderPass_ = std::make_unique<Vulkan::RenderPass>(swapChain, depthBuffer, false, false);
...
//		Throw(std::runtime_error("failed to initialise ImGui vulkan adapter"));
	...
	// Upload ImGui fonts
	if (!io.Fonts->AddFontFromFileTTF("../resources/fonts/Cousine-Regular.ttf", 13 * scaleFactor))
	{
