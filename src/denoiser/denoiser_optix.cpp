...
void DenoiserOptix::denoiseImage(vulkan::Device& device,
    ...
    OptixImage2D input_layer = {
        (CUdeviceptr)pixel_buffer_in_.cuda_ptr,
        image_size_.width, image_size_.height,
        image_size_.width * size_of_pixel,
        size_of_pixel, pixel_format
    };

    OptixImage2D output_layer = {
        (CUdeviceptr)pixel_buffer_out_.cuda_ptr,
        image_size_.width, image_size_.height,
        image_size_.width * size_of_pixel,
        size_of_pixel, pixel_format
    };
...
void DenoiserOptix::allocateBuffers(vulkan::Device& device) {
    ...
    OPTIX_CHECK(optixDenoiserComputeMemoryResources(denoiser_,
                                                    image_size_.width,
                                                    image_size_.height,
                                                    &denoiser_sizes_));
...
//
    ...
    cuda_buffer.handle = vkDevice.getMemoryWin32HandleKHR({cuda_buffer.buf_vk.allocation,
